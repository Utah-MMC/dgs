#!/usr/bin/env python3
"""
Restore content to all pages EXCEPT the 4 core pages:
- /services/seo/
- /services/paid-search/
- /services/creative/
- /services/hubspot/

These core pages should keep their existing content.
"""
...
# Core pages to skip (keep their existing content)
CORE_PAGES = {
...
def generate_comprehensive_content(title, description, page_type="service"):
    ...
    clean_title = title.replace(' - Digital Growth Studios', '').replace('Digital Growth Studios - ', '').strip()
...
def restore_page_content(file_path):
    ...
    try:
        ...
        # Create new content section with proper attributes matching homepage style
        new_fullwidth = soup.new_tag('div')
        new_fullwidth['class'] = 'fusion-fullwidth fullwidth-box'
        # Add styling to match homepage format - white background with padding
        ...
        # Wrap content in fusion-text div for proper styling
        fusion_text = soup.new_tag('div')
        fusion_text['class'] = 'fusion-text'
        fusion_text['style'] = 'margin-top:40px;'
        
        # Insert generated content into fusion-text
        fusion_text.append(content_soup)
        new_wrapper.append(fusion_text)
...
def main():
    ...
    print(f"{'='*70}")
    print(f"Restore Content (Except 4 Core Pages)")
    print(f"{'='*70}")
    ...
    for i, html_file in enumerate(html_files, 1):
        ...
        result = restore_page_content(html_file)
        ...
        if result == 'restored':
            ...
            print(f"  - Skipped (core page - keeping existing content)")
        elif result == 'skipped_has_content':
            stats['skipped_has_content'] += 1
            print(f"  - Skipped (already has content)")
        elif result == 'skipped_no_metadata':
            stats['skipped_no_metadata'] += 1
            print(f"  - Skipped (missing title or description)")
    ...
    print(f"  ✓ Restored: {stats['restored']} pages")
    print(f"  - Skipped (core pages): {stats['skipped_core']} pages")
    print(f"  - Skipped (has content): {stats['skipped_has_content']} pages")
    print(f"  - Skipped (no metadata): {stats['skipped_no_metadata']} pages")
